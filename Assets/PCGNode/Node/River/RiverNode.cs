...
namespace PCGNode
{
    public class RiverNode
    {
        ...
        public static void fill_lake(ref float[] elevations, ref float[] fills, ref int[] mask, int width)
        {
            for (int i = 0; i < width; i++)
            {
                for (int j = 0; j < width; j++)
                {
                    if (mask[i + j * width] != 0) elevations[i + j * width] = 1.0f;
        ...
        private static void calc_flow_directions(ref float[] elevations, ref int[] FD, int width, bool write_flat = false)
        {
            for (int i = 0; i < width; i++)
            {
                for (int j = 0; j < width; j++)
        ...
        private static void gradient_towards_lower(ref float[] elevations, ref List<Vector2Int> flats, ref int[] flows, ref int[] inc1, int width)
        {
            int loops = 0;
        ...
        private static void gradient_from_higher(ref float[] elevations, ref List<Vector2Int> flats, ref int[] flows, ref int[] inc2, int width)
        {
        ...
        private static void redirect_flats(ref float[] elevations, ref List<Vector2Int> flats, ref int[] inc1, ref int[] inc2, ref int[] FD, int width)
        {
        ...
        {
            List<Vector2Int> flats = new List<Vector2Int>();
            for (int i = 0; i < width; i++)
                for (int j = 0; j < width; j++)
                    if (FD[i + j * width] == NO_FLOW)
                        flats.Add(new Vector2Int(i, j));
        ...
        public static float[] GenerateRiver(float[] elevations, int width, int[] dry_mask, int[] river_mask)
        {
            ...
            fill_lake(ref elevations, ref filled_elevations, ref dry_mask, width);
